...
function response(room, msg, sender, isGroupChat, replier) {
  ...
  // 팩토리얼
  var number;
  var factorialHTML = Jsoup.connect("https://ko.numberempire.com/factorialcalculator.php?number=" + number).userAgent(USER_AGENT).get();
  var factorialResult = String(factorialHTML.select("span#result1")).replace(/(<([^>]+)>)/ig, "");

  // 소인수분해
  var number;
  var factorizerHTML =  Jsoup.connect("https://ko.numberempire.com/numberfactorizer.php?number=" + number).userAgent(USER_AGENT).get();
  ...
  var number; // 검사할 숫자
  var action; // 소수 검사 동작
  var primeNumbersHTML =  Jsoup.connect("https://ko.numberempire.com/primenumbers.php?number=" + number + "&action=" + ACTION_MAP.get(action)).userAgent(USER_AGENT).get();
  ...
  replier.reply(primeNumbersResult)
}
